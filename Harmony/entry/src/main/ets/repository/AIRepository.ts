...
import { DeepseekRequest, Message } from '../network/model/DeepseekRequest';
...
export class AIRepository {
  ...
  async sendMessage(userId: number, message: string): Promise<string> {
    ...
    // 添加系统提示
    requestMessages.push(new Message(AppConstants.ROLE_SYSTEM, 
      getString('ai_system_prompt')));
   ...
   * 获取聊天历史
   */
  async getChatHistory(userId: number): Promise<Array<{ role: string, content: string, messageTime: number }>> {
    await this.ensureInit();
    ...
    const history: Array<{ role: string, content: string, messageTime: number }> = [];
    while (resultSet.goToNextRow()) {
      history.push({
        role: resultSet.getString(resultSet.getColumnIndex('role')),
        content: resultSet.getString(resultSet.getColumnIndex('content')),
