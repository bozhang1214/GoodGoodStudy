...
export class UserRepository implements IUserRepository {
   ...
   */
  async register(username: string, password: string, nickname: string): Promise<{ userId: number, user: User }> {
    ...
    try {
      ...
      return new Promise<{ userId: number, user: User }>((resolve, reject) => {
        rdbStore.createTransaction().then((transaction) => {
          try {
            ...
            try {
              ...
              Logger.warnWithTag('UserRepository', 'Failed to rollback transaction', rollbackError as Error);
            }
            // 使用统一错误处理
            const handledError = ErrorHandler.handleError(error, 'register user', 'database');
            reject(handledError);
          }
        }).catch((err) => {
      ...
      // 使用统一错误处理
      throw ErrorHandler.handleError(error, 'register user', 'database');
    }
  }
   ...
   * 回退的插入方法（当事务不可用时使用）
   */
  private async fallbackInsertUser(user: User, username: string, resolve: (value: { userId: number, user: User }) => void, reject: (reason?: any) => void): Promise<void> {
    try {
      ...
      // 使用统一错误处理
      const handledError = ErrorHandler.handleError(error, 'fallback insert user', 'database');
      reject(handledError);
    }
  }
